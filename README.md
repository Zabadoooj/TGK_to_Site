...
2. Получаем API ключ и создаем .env файл.
...
3. Получаем и вставляем в тот-же файл ID нашего пользователя в строку `USER_ID = `
    
    узнать его можно в любом боте по типу ([@re_GetMyIDBot](https://t.me/re_GetMyIDBot))
4.  Указываем в `TG_CHANNEL = @` тег нашего канала (например `TG_CHANNEL = @zabadooj_cuts`)

5. В [файле конфигурации фронтенда](./frontend/src/_config.js) задаете диапозон от какого и до какого сообщения вам нужно получить

6. Устанавливаем зависимости для бэка
```
cd ./bacend/bot
```
